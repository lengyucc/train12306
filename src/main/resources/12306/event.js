TouClick.ready(function () {
    var touclick_start = function (randCode_name, code_type, cssConfig, rand, targetdiv) {
        var touclick = TouClick.get("touclick-" + randCode_name).start({
            // captcha pic url
            gp_url: ctx + "passcodeNew/getPassCodeNew?module=" + code_type + "&rand=" + rand,
            // click callback
    ...
    for (var i = 0; i < targetelement.length; i++) {
        ...
        var $targetdiv = $('#' + targetdiv[i]),
            code_type = $targetdiv.data("code_type"),
    ...
    $('.captchaFloatButton').each(function (i, v) {
        ...
        leftTicketOrderNote.parent().css({
            "height": "25px"
        });
