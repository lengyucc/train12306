        ...
        $.ajax({
            ...
            error: function (cs, cu, ct) {
                ...
                if (cu.status) {
                    ...
                    if (cu.data.flag == 1) {
                        cu.data = b4(cu.data.result, cu.data.map)   // 把车次信息放入cu.data
                    }
                    TAG2
                    trainListForIE = [];
    ...
    showTicketPrice = function (cu) {
        ...
        if (cs && ($("#ticket_" + cv + "_train>span>span").text() == "查看票价")) {
            ...
            $.ajax({
                ...
                timeout: 1000,
                error: function (cA, cC, cB) {}, success: function (cA) {}
            });
            $.ajax({
                type: "get",
                isTakeParam: false,
    ...
    function bZ(cq) {
        if (isSaveQueryLog == "Y") {
            $.ajax({
                ...
                data: cq,
                timeout: 15000,
                error: function (cr, ct, cs) {}, success: function (cr) {}
            })
        }
    }
    var aT = 0;
